...
options=$(getopt -o "hdt:s:u:" -l "help,host:,default,type:,user:,pass:" -- "$@") || { usage; exit 1; }
eval set -- "$options"
...
script_response=$(curl -s -i -H "apikey: $apikey" -H 'Accept: application/json' "http://$host/api/v3/Renamer/Config/$script_name_url_encoded")
if printf %s "$script_response" | head -n 1 - | grep -q '200'; then
  ...
  update_response=$(curl -s -i -H "apikey: $apikey" -H 'Content-Type: application/json' -X PUT -d "$new_script_json" "http://$host/api/v3/Renamer/Config/$script_name_url_encoded")
...
if printf %s "$update_response" | head -n 1 - | grep -q '200'; then
  ...
  if $default; then
    patch_json=$(jq --null-input --arg name "$script_name" '[{"op":"replace", "path":"/Plugins/Renamer/DefaultRenamer", "value": $name }]')
