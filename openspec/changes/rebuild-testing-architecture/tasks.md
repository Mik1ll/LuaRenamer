...
**Finding (production defect, not fixed here — this change makes no production behavior changes):**
`GetExistingAnimeLocation` filters candidate locations with
`DropFolderType.HasFlag(Destination) || DropFolderType.HasFlag(Excluded)`, but `DropFolderType.Excluded` is `0`, so `HasFlag(Excluded)` is true for every value and the second clause admits *any* folder — a Source-only folder qualifies as an existing anime location. Surfaced by 7.3 while arranging the "no candidate qualifies" case. The fallback test was rewritten to use a candidate that genuinely does not qualify (a file at a folder root, so no subfolder to reuse) rather than pin the current behavior.
...
- **8.2** 20 consecutive runs green, plus `-parallelMode all -maxThreads 16` and randomized execution order (`:seed`). No order dependence.
...
- **8.4** The workflow's steps run green locally with `CI=true`: `dotnet restore --locked-mode`, `dotnet build --no-restore`, `git diff --exit-code -- LuaRenamer/lua/` (no defs drift), `dotnet test --no-restore --no-build --verbosity normal` (182 passed).

**Finding (production defect, not fixed here):** `Utils.NormPath` is built on `Path.TrimEndingDirectorySeparator`, which trims exactly one trailing separator. A path ending in repeated separators therefore stays separator-terminated (`"a\\"` -> `"a\"`) and normalizing again changes it, so `NormPath` is neither idempotent nor trailing-separator-free for that input. Surfaced by the 3.5 properties. The properties exclude that input explicitly, with a comment naming this finding, rather than relaxing what they assert.
